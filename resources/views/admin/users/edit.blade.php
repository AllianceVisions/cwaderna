                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2 {{ $errors->has('city_id') ? 'is-invalid' : '' }}" name="city_id" id="city_id" required>
                                    @foreach($cities as $id => $name)
                                        <option value="{{ $id }}"  @if($user->city_id == $id) selected @endif {{ old('city_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6"> 
                            <div class="form-group">
                                ...
                                <input class="form-control {{ $errors->has('national_id') ? 'is-invalid' : '' }}" type="text" name="national_id" id="national_id" value="{{ old('national_id', $user->national_id) }}" required>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2 {{ $errors->has('nationality') ? 'is-invalid' : '' }}" name="nationality" id="nationality" required>
                                    ...
                                    @foreach(App\Models\User::NATIONALITY_SELECT as $label)
                                        <option value="{{ $label }}" {{ old('nationality', $user->nationality) === (string) $label ? 'selected' : '' }}>{{ trans('global.nationality.'.$label) }}</option>
                                    @endforeach
                                </select>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2 {{ $errors->has('events') ? 'is-invalid' : '' }}" name="events[]" id="events" multiple>
                            @foreach($events as $event)
                                <option value="{{ $event->id }}" {{ (in_array($event->id, old('events', [])) || $user->events->contains($event->id)) ? 'selected' : '' }}>{{$event->title}} ({{ $event->event_organizer ? $event->event_organizer->company_name : "" }}) </option>
                            @endforeach
                        </select>
