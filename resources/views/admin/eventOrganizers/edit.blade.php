                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control {{ $errors->has('gender') ? 'is-invalid' : '' }}" name="gender" id="gender" required>
                                    ...
                                    @foreach(App\Models\User::GENDER_SELECT as $key => $label)
                                        <option value="{{ $key }}" {{ old('gender', $eventOrganizer->user->gender) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
...
<script>
    Dropzone.options.identityDropzone = {
        ...
        init: function () {
    @if(isset($eventOrganizer->identity))
        var file = {!! json_encode($eventOrganizer->identity) !!}
            this.options.addedfile.call(this, file)
        file.previewElement.classList.add('dz-complete')
...
<script>
    Dropzone.options.commericalregDropzone = {
        ...
        init: function () {
    @if(isset($eventOrganizer->commerical_reg) )
        var file = {!! json_encode($eventOrganizer->commerical_reg) !!}
            this.options.addedfile.call(this, file)
        file.previewElement.classList.add('dz-complete')
