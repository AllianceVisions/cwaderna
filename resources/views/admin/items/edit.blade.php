...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.items.update", [$item->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category_id" id="category_id" required>
                    @foreach($categories as $id => $entry)
                        <option value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $item->category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label class="required" for="provider_man_id">{{ trans('cruds.item.fields.provider_man') }}</label>
                <select class="form-control select2 {{ $errors->has('provider_man') ? 'is-invalid' : '' }}" name="provider_man_id" id="provider_man_id" required>
                    @foreach($provider_men as $id => $entry)
                        <option value="{{ $id }}" {{ (old('provider_man_id') ? old('provider_man_id') : $item->provider_man->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('provider_man'))
                    <div class="invalid-feedback">
                        {{ $errors->first('provider_man') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.item.fields.provider_man_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number" name="price" id="price" value="{{ old('price', $item->price) }}" step="0.01" required>
                @if($errors->has('price'))
                    <div class="invalid-feedback">
                        {{ $errors->first('price') }}
                    </div>
...
@if(isset($item) && $item->photo)
      var file = {!! json_encode($item->photo) !!}
