...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('events-organizer.events.store') }}" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="event_organizer_id" value="{{ auth()->user()->events_organizer->id }}">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="row"> 
                            <div class="col-md-8"> 
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2 {{ $errors->has('city_id') ? 'is-invalid' : '' }}"
                                        name="city_id" id="city_id" required>
                                        @foreach ($cities as $id => $name)
                                            <option value="{{ $id }}"
                                                {{ old('city_id', '') == $id ? 'selected' : '' }}>{{ $name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="form-group">
                                    <label class="required"
                                        for="area">{{ trans('cruds.event.fields.area') }}</label>
                                    <input class="form-control {{ $errors->has('area') ? 'is-invalid' : '' }}"
                                        type="number" step="0.00000001" name="area" id="area"
                                        value="{{ old('area', 1000) }}" required>
                                    @if ($errors->has('area'))
                                        <div class="invalid-feedback">
                                            {{ $errors->first('area') }}
                                        </div>
                        ...
                        <div class="form-group">
                            <label class="required"
                                for="conditions">{{ trans('cruds.event.fields.conditions') }}</label>
                            <textarea class="form-control {{ $errors->has('conditions') ? 'is-invalid' : '' }}"
                                name="conditions" id="conditions" required>{{ old('conditions', '') }}</textarea>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    ...
                                    @endif
                                    <span
                                        class="help-block">{{ trans('cruds.event.fields.start_date_helper') }}</span>
                                </div>
                                {{-- start_attendance --}}
                                <div class="form-group">
                                    <label class="required"
                                        for="start_attendance">{{ trans('cruds.event.fields.start_attendance') }}</label>
                                    <input
                                        class="form-control timepicker {{ $errors->has('start_attendance') ? 'is-invalid' : '' }}"
                                        type="text" name="start_attendance" id="start_attendance"
                                        value="{{ old('start_attendance') }}" required>
                                    ...
                                    @endif
                                    <span
                                        class="help-block">{{ trans('cruds.event.fields.start_attendance_helper') }}</span>
                                </div>
                            </div>
                            <div class="col-md-6">
                                {{-- end_date --}}
                                <div class="form-group">
                                    <label class="required"
                                        for="end_date">{{ trans('cruds.event.fields.end_date') }}</label>
                                ...
                                <div class="form-group">
                                    <label class="required"
                                        for="end_attendance">{{ trans('cruds.event.fields.end_attendance') }}</label>
                                    <input
                                        class="form-control timepicker {{ $errors->has('end_attendance') ? 'is-invalid' : '' }}"
                                        type="text" name="end_attendance" id="end_attendance"
                                        value="{{ old('end_attendance') }}" required>
                                    ...
                                    @endif
                                    <span
                                        class="help-block">{{ trans('cruds.event.fields.end_attendance_helper') }}</span>
                                </div>
                            </div>
                        </div>
...
@section('scripts')
    <script src="https://maps.google.com/maps/api/js?key=AIzaSyCq2UTcMzYp__KQb0P_By0dmzCjP9Twors&libraries=places&v=weekly">
    </script>
    <script>
        locate();
