    ...
    <ul class="c-header-nav @if(app()->getLocale() == 'ar') mr-auto @else ml-auto @endif">
        ...
        <ul class="c-header-nav ml-auto">
                    ...
                    @if(count($alerts = \Auth::user()->userUserAlerts()->withPivot('read')->limit(10)->orderBy('created_at', 'ASC')->get()->reverse()) > 0)
                        @foreach($alerts as $alert)
                            <div class="dropdown-item">
                                @if($alert->type == 'event')
                                    <a href="{{ route('events-organizer.events.show',$alert->alert_link)}}" onclick="notificationRead('{{$alert->id}}')" rel="noopener noreferrer">
            ...
            <li class="c-header-nav-item dropdown d-md-down-none" style=" background: #EBEDEF; border-radius: 8px 39px 0px 0px; padding: 0px 13px;">
                <a class="c-header-nav-link" data-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false">
                    @if(auth()->user()->photo)
                        <img src="{{auth()->user()->photo->getUrl('thumb')}}" alt="" width="40" height="40" style="border-radius: 50px;margin:10px">
                    @else 
                ...
                <div class="dropdown-menu dropdown-menu-right"> 
                    <a class="dropdown-item" href="{{route('events-organizer.profile.edit')}}">{{trans('global.profile')}}</a> 
